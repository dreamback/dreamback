...
(function (window, undefined) {
    ...
    dreamback.prototype = {
        ...
        before: function(elem,target){
            dream.before(this[0],elem,target);
            return this;
        },
        ...
        html: function (html) {
            if (html != undefined) {
                this.empty().insertHTML(html, 'beforeEnd');
                return this;
            }
            return this[0].innerHTML;
        },
        text: function () {
            return dream.text(this[0]);
        },
            ...
            return this.each(function () {
                ...
                dream.each(style, function (key) {
                    if(!/color/i.test(key)){
                        ...
                        start[key] = dream.parseColor(dream.getStyle(elem, key));
                        s = dream.parseColor(this[key]);
                        e = dream.parseColor(start[key]);
                        alter[key] = {
                            r: s.r-e.r,
                            g: s.g-e.r,
                            b: s.b-e.b
        ...
        val: function (value) {
            if (value == undefined) {
                return dream.val(this[0]);
            }
            return this.each(function () {
                dream.val(this, value);
            });
        },
        focus: function () {
            return this.each(function () {
    ...
    dream.extend({
        ...
        find: function (name, target) {
            if (name.indexOf('#') > -1) {
                return this.getId(name.replace('#', ''));
            } else if (name.indexOf('.') > -1) {
                return this.getByClass(name.replace('.', ''), target);
            } else {
        ...
        before: function(parent,newElem,target){
            target = target || parent.firstChild;
            parent.insertBefore(newElem,target);
        },  
        ...
        val: function (elem, value) {
            ...
            if (value == undefined) {
                ...
                if (type === "radio" || type === "checkbox") {
                    ...
                    val = elem.selectedIndex > -1 ? dream.each(elem.options, function (elem) {
        ...
        text: function (elem) {
            ...
            var els = elem.getElementsByTagName('*'),
                ...
                text = '';
            for (; i < len; i++) {
                if (3 == els[i].firstChild.nodeType) {
        ...
        animate: function (elem, start, alter, duration, callBack, tween) {
            var startTime = +new Date(), // 动画开始的时间
                tween = tween || dream.Easing.linear,
                timeout = null,
                ...
                run = function () {
                    ...
                    dream.each(start, function (key) {
                        if(!/color/i.test(key)){
                            value = (start[key] + (alter[key]) * e).toFixed(3);
                        }else{
                            rgb.r = parseInt(start[key].r + alter[key].r*e);
                            rgb.g = parseInt(start[key].g + alter[key].g*e);
                            rgb.b = parseInt(start[key].b + alter[key].b*e);
                        }
                        
                        if (key == 'opacity') {
                            dream.setStyle(elem, 'opacity', value);
                        }else if(/color/i.test(key)){
                            dream.setStyle(elem, key , 'rgb(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ')');
                        }else{
    ...
    dream.addEvent = function (elem, type, handler) {
        var $handler = function (event) {
            handler.call(elem, event || window.event);
        }
    ...
    dream.removeEvent = function (elem, type, handler) {
    ...
    dream.getWheelDelta = function (event) {
        return event.wheelDelta ? -event.wheelDelta / 120 : (event.detail > 0 ? 1 : -1);
    }
    ...
    function ajax(options) {
        ...
        this.waiting = options.waiting || function () {};
        this.stopWaiting = options.stopWaiting || function () {};
